...
const Blog = new Schema ({
    ...
    comment: [{comments: {type: String, required: false}, username: {type: String, required: false},
...
Blog.methods.likeblog = function (blogid,userid) {
...
Blog.methods.docomment = function (comment,username,userid,date) {
 ...
 Blog.methods.deleteComment = function (commentId,userid) {
    const Commenting = [...this.comment]; 
    let f;
    for(let i=0;i<Commenting.length; i++) {
        if(Commenting[i]._id.toString() === commentId.toString()) {
            f=i;
            break;
        }
    }
    Commenting.splice(f,1);
    this.comment = Commenting;
    return this.save();
 }
